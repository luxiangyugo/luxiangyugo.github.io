...
The site follows the original academic homepage structure: ordinary navigation, a short introduction with the original portrait, brief news, and selected publications. A pale gray-green background, translucent navigation and profile links, and quiet edge highlights add depth without changing the content hierarchy. Publication lists remain flat and readable. Research, Publications, Patents, CV and Hobbies have separate pages; contact information is on Home/CV/footer. All pages are static and work without JavaScript.
...
PaddySeg and direct geo-location are separate entries as on the old page; the latter is explicitly a companion method from the same paper. Publication years follow the verified bibliography. The current optical projects, numbered 15 and 14, use compact conceptual lens illustrations; actual archived outputs remain linked through “Prototype outputs”. Their layout, 15px body text, and short descriptions match historical entries. Mobile layouts show each title, illustration, and technical description in sequence. Research funding sits between the overview and Current research. See [technical copy sources](research-copy-sources.md) for the published results used in the summaries.

Patents are managed in `_data/patents.yml` and shown at `/patents`, after Publications in navigation; the second heading uses the requested “Utility patents” label. CV shows brief experience, a short Chinese biography, and direct downloads for the dated English and Chinese PDFs. `_data/cv_zh.yml` retains source notes, but only its download path and date are rendered. Contact information is consolidated into Home/CV/footer, with a short `/contact` compatibility page. See [patent and CV sources](patents-and-cv-sources.md).
